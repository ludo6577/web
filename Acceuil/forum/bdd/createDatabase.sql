CREATE TABLE groupe (idgrp serial PRIMARY KEY, libgrp character varying(20));
CREATE TABLE  usr(iduser serial primary key, loginuser varchar(30) NOT NULL, passwduser varchar(15) NOT NULL, emailuser varchar(100), dateinsuser date, idgrp bigint unsigned, foreign key (idgrp) references groupe(idgrp));

CREATE TABLE theme (idthm serial primary key, libthm varchar(100) NOT NULL, datecrthm date, iduserthm bigint unsigned NOT NULL, idsuperthm bigint unsigned, FOREIGN KEY (iduserthm) REFERENCES usr (iduser), FOREIGN KEY (idsuperthm) REFERENCES theme (idthm));

CREATE TABLE message ( idmsg serial PRIMARY KEY, titremsg varchar(100) NOT NULL, contenumsg text NOT NULL, datemsg date, idthmmsg bigint unsigned NOT NULL, idmsgsrc bigint unsigned, idusermsg bigint unsigned, lat decimal(10,7), lon decimal(10,7),
...
INSERT INTO usr (loginuser, passwduser, emailuser, dateinsuser, idgrp) VALUES('admin','admin','admin@esiee.fr','2013-02-10',1);
INSERT INTO usr (loginuser, passwduser, emailuser, dateinsuser, idgrp) VALUES('tabc','ptabc','tabc@unvi-mlv.fr','2013-11-20',2);
INSERT INTO usr (loginuser, passwduser, emailuser, dateinsuser, idgrp) VALUES('tdef','ptdef','tdef@univ-mlv.fr','2013-12-25',2);
INSERT INTO usr (loginuser, passwduser, emailuser, dateinsuser, idgrp) VALUES('eabc','peabc','eabc@univ-mlv.fr','2013-12-10',2);
INSERT INTO usr (loginuser, passwduser, emailuser, dateinsuser, idgrp) VALUES('edef','pedef','edef@univ-mlv.fr','2013-12-12',3);
INSERT INTO usr (loginuser, passwduser, emailuser, dateinsuser, idgrp) VALUES('eghi','peghi','egi@univ-mlv.fr','2013-12-13',3);
